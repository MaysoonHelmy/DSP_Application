...
class TaskPage4(tk.Frame):
    ...
    def process_signal(self):
        ...
        if select == "DFT":
            ...
            amplitude=[float(x) for x in amplitude]
            ...
            components = np.array(x) * np.exp(1j * np.array(self.input_samples)) 
            reconstructed_signal = self.compute_fourier(components, N, inverse=True)
            amplitude, phase = self.calculate_amplitude_and_phase(reconstructed_signal )
            amplitude = [round(x) for x in amplitude]
            indices = [i for i in range(len(amplitude))]
            freqs = [k * sampling_frequency / N for k in range(N)]
            print(indices)
            print (amplitude)
            self.plot_signal( indices, amplitude,"IDFT signal")
    ...
    def delay_signal(self):
            ...
            if not self.input_file:
                ...
                return
            operation = self.operation_var.get()
            if operation == "Delay Folded Signal":
               self.input_samples , x =self.fold_signal()
            else:  
              self.input_samples, x = self.read_file_space(self.input_file)
    ...
    def advance_signal(self):
        ...
        if not self.input_file:
            ...
            return
        operation = self.operation_var.get()
        if operation == "Advance Folded Signal":
            self.input_samples , x =self.fold_signal()
        else:  
          self.input_samples , x = self.read_file_space(self.input_file)
        if self.input_samples is None:
